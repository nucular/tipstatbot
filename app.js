...
client.on("message#", function(from, to, text, message) {
    ...
    if (m && m.length > 0 && cmds.hasOwnProperty(name)) {
        ...
        if (cmd.costblock && costs > 0) {
            client.notice(from, from + ": Please wait " + String(costs) + " seconds.");
            return;
        }
        ...
        for (var i = 0; i < cmd.args.length; i++) {
            if (i >= rargs.length) {
                if (cmd.args[i].optional) {
                    ...
                    client.notice(from, from + ": " + "Not enough arguments! Try "
            ...
            if (cmd.args[i].type == "number") {
                arg = Number(arg);
                if (isNaN(arg)) {
                    client.notice(from, from + ": " + cmd.args[i].name.toUpperCase() + " has to be a number! Try "
                        + BOT_PREFIX + "help " + name);
                    return;
                }
                ...
                if (!util.isValidDate(arg)) {
                    client.notice(from, from + ": " + cmd.args[i].name.toUpperCase() + " has to be a valid date (with a time value)! Try "
                        + BOT_PREFIX + "help " + name);
                    return;
                }
            } else {
        ...
        if (cmd.restricted) {
            ...
            if (from != BOT_OWNER) {
                ...
                client.notice(from, from + ": Sorry, that command is reserved for " + BOT_OWNER + ".");
            } else {
                var cb = function(nnick, nto, ntext, nmessage) {
                    if (nnick == "NickServ") {
                        ...
                        if (ntext.indexOf(BOT_OWNER + " ACC 3") != -1) {
                            ...
                            debug("Impersonation!");
                            client.notice(from, from + ": Stop impersonating my owner!");
                        }
                    }
                }
                client.on("notice", cb);
